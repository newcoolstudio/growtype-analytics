...
class Growtype_Analytics_Admin_Metrics
{
    ...
    public function get_scale_or_pivot_metrics($refresh = false, $period = 30)
    {
        ...
        $metrics['new_user_to_buyer_conversion'] = $this->controller->users_metrics->get_new_user_to_buyer_conversion($period, $settings);
        $metrics['new_user_to_buyer_conversion_daily'] = $this->controller->users_metrics->get_new_user_to_buyer_conversion(1, $settings);
        ...
        $metrics['attempt_users'] = $this->controller->users_metrics->get_new_attempt_users_count($period, $settings);
        $metrics['user_try_to_buy_rate'] = $metrics['new_users'] > 0
            ? round(($metrics['attempt_users'] / $metrics['new_users']) * 100, 2)
            : 0;

        $repurchase = $this->controller->users_metrics->get_repurchase_metrics($settings);
        $metrics['repurchase_rate_total'] = $repurchase['rate'];
        $metrics['arppu_total'] = $repurchase['arppu'];

        $activity = $this->controller->users_metrics->get_activity_metrics($settings);
        ...
        $metrics['churn_risk_recent_payers'] = $this->controller->users_metrics->get_churn_risk_count($settings);
        ...
        $metrics['payer_churn_rate'] = $this->controller->users_metrics->get_payer_churn_rate($settings);

        // User Churn Rate: % of active users in prev 30d who are NOT active in current 30d
        $metrics['user_churn_rate'] = $this->controller->users_metrics->get_user_churn_rate($settings);

        // Median days from registration to first purchase
        $metrics['median_days_to_first_purchase'] = $this->controller->users_metrics->get_median_days_to_first_purchase($settings);
        ...
        $new_buyers_30d = $this->controller->users_metrics->get_new_buyers_count($period, $settings);
